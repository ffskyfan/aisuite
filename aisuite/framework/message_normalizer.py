...
class MessageNormalizer:
    ...
    PROVIDER_FIELD_MAP = {
        ...
        "gemini": {"remove_fields": [], "preserve_reasoning": False},
    }
    ...
    PROVIDER_CACHE_SUPPORT = {
        "anthropic": {  # 使用正确的provider名称
            ...
            "format": lambda config: (
                {
                    "type": "ephemeral",
                    "ttl": str(config.get("ttl", "5m"))
                    .replace("CacheTTL.", "")
                    .replace("MINUTES_5", "5m")
                    .replace("HOURS_1", "1h"),
                }
                if config.get("type")
                in ["ephemeral", CacheType.EPHEMERAL, "CacheType.EPHEMERAL"]
                else None
            ),
            "min_tokens": {
                "claude-3-opus": 1024,
    ...
    @classmethod
    def _convert_cache_config(
        cls, cache_config: Any, target_provider: str, model_name: str = None
    ) -> Optional[Dict]:
        """
    ...
    @classmethod
    def _apply_cache_marking(
        cls, message: Dict, target_provider: str, model_name: str = None
    ) -> Dict:
        ...
        provider_cache = cls._convert_cache_config(
            cache_config, target_provider, model_name
        )
    ...
    def normalize_message(
        cls, message: Union[Dict, Message], target_provider: str = None
    ) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def _clean_message(
        cls, msg_dict: Dict[str, Any], target_provider: str = None
    ) -> Dict[str, Any]:
        ...
        # Some providers can preserve reasoning content for multi-turn reasoning continuity.
        if preserve_reasoning:
            always_remove = {
                "refusal"
            }  # Some OpenAI models can handle reasoning_content
        ...
        for key, value in msg_dict.items():
            ...
            if key == "reasoning_content":
                # Only preserve for providers that explicitly support it.
                if preserve_reasoning and value:
                    cleaned[key] = value
                continue
    ...
    def normalize_messages(
        cls, messages: List[Union[Dict, Message]], target_model: str = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        target_provider = (
            cls.detect_provider_type(target_model) if target_model else None
        )
        normalized = []
        ...
        for msg in messages:
            ...
            if isinstance(msg, dict):
                ...
                normalized_msg = cls._apply_cache_marking(
                    normalized_msg, target_provider, target_model
                )

                # For assistant messages, also apply other normalizations
                if normalized_msg.get("role") == "assistant":
                    normalized_msg = cls.normalize_message(
                        normalized_msg, target_provider
                    )
                else:
    ...
    def extract_reasoning_content(
        cls, message: Union[Dict, Message]
